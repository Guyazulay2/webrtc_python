...
class Camera:
    ...
    def start(self):
        ...
        height = 410
        pipe = ""
        pipe += f"multifilesrc location=\"outputs/00/frame%d.png\" index=1 caps=\"image/png,framerate=30/1\" ! decodebin ! videoscale ! capsfilter caps=\"video/x-raw, width=600, height=400\" ! compositor.sink_0 "
        pipe += f"multifilesrc location=\"outputs/01/frame%d.png\" index=1 caps=\"image/png,framerate=30/1\" ! decodebin ! videoscale ! capsfilter caps=\"video/x-raw, width=600, height=400\" ! compositor.sink_1 "
        pipe += f"multifilesrc location=\"outputs/02/frame%d.png\" index=1 caps=\"image/png,framerate=30/1\" ! decodebin ! videoscale ! capsfilter caps=\"video/x-raw, width=600, height=400\" ! compositor.sink_2 "
        pipe += f"multifilesrc location=\"outputs/03/frame%d.png\" index=1 caps=\"image/png,framerate=30/1\" ! decodebin ! videoscale ! capsfilter caps=\"video/x-raw, width=600, height=400\" ! compositor.sink_3 "
        pipe += f"multifilesrc location=\"outputs/10/frame%d.png\" index=1 caps=\"image/png,framerate=30/1\" ! decodebin ! videoscale ! capsfilter caps=\"video/x-raw, width=600, height=400\" ! compositor.sink_4 "
        pipe += f"multifilesrc location=\"outputs/11/frame%d.png\" index=1 caps=\"image/png,framerate=30/1\" ! decodebin ! videoscale ! capsfilter caps=\"video/x-raw, width=600, height=400\" ! compositor.sink_5 "
        pipe += f"multifilesrc location=\"outputs/12/frame%d.png\" index=1 caps=\"image/png,framerate=30/1\" ! decodebin ! videoscale ! capsfilter caps=\"video/x-raw, width=600, height=400\" ! compositor.sink_6 "
        pipe += f"multifilesrc location=\"outputs/13/frame%d.png\" index=1 caps=\"image/png,framerate=30/1\" ! decodebin ! videoscale ! capsfilter caps=\"video/x-raw, width=600, height=400\" ! compositor.sink_7 "
